...
public class GameToolsPlayerApiClient
        implements ServerPlayersClient, UserStatsClient {
    ...
    public ServerPlayers fetchServerPlayers(String serverId) throws ApiRequestException {
        try {
            ...
            JsonNode teams = root.path(ApiConstants.JSON_TEAMS);
            if (teams.isArray()) {
                for (JsonNode team : teams) {
                    String teamName = team.path("name").asText("Unknown Team");
                    for (JsonNode p : team.path(ApiConstants.JSON_PLAYERS)) {
                        players.add(new ServerPlayer(
                                p.path(ApiConstants.JSON_NAME).asText("Unknown"),
                                p.path(ApiConstants.JSON_PLAYER_ID).asLong(0),
                                p.path(ApiConstants.JSON_USER_ID).asLong(0),
                                p.path(ApiConstants.JSON_PLATOON).asText(""),
                                p.path(ApiConstants.JSON_RANK).asInt(0),
                                teamName
                        ));
                    }
                }
    ...
    public UserStats fetchUserStats(String playerName) throws ApiRequestException {
        try {
            ...
            return new UserStats(
                    json.path(ApiConstants.JSON_USERNAME).asText(playerName),
                    json.path("userId").asLong(0),
                    json.path("id").asLong(0),
                    json.path(ApiConstants.JSON_RANK).asInt(0),
                    json.path(ApiConstants.JSON_KILLS_PER_MINUTE).asDouble(0.0),
                    json.path(ApiConstants.JSON_ACCURACY).asText("0%"),
                    json.path(ApiConstants.JSON_HEADSHOTS).asText("0%"),
                    json.path(ApiConstants.JSON_TIME_PLAYED).asText("Unknown"),
                    json.path(ApiConstants.JSON_SECONDS_PLAYED).asLong(0),
                    json.path(ApiConstants.JSON_KILLS).asInt(0),
                    json.path(ApiConstants.JSON_DEATHS).asInt(0),
                    json.path(ApiConstants.JSON_KILL_DEATH).asDouble(0.0)
            );
        } catch (ApiRequestException e) {
            throw e;
        } catch (Exception e) {
