...
public class StatusPanel extends HBox {
    ...
    /**
     * Configure the timer for displaying elapsed time
     */
        ...
        timer.getKeyFrames().add(
            new KeyFrame(Duration.seconds(0.5), event -> {
                if (scanStartTime != null) {
                    java.time.Duration elapsed = java.time.Duration.between(scanStartTime, Instant.now());
                    long minutes = elapsed.toMinutes();
                    long seconds = elapsed.minusMinutes(minutes).getSeconds();
                    timeLabel.setText(String.format("%dm %02ds", minutes, seconds));
                }
            })
        );
    }
    ...
    /**
     * Stop the timer and set the final time
     */
    ...
    /**
     * Get the time label for use in other components
     */
